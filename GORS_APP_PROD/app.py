...
def parse_kite_csv(raw):
    ...
    pnl_col = find_col(["p&l", "pnl", "profit_and_loss", "profit_loss"])
    ...
    if missing:
        ...
        raise ValueError("Missing required Kite columns: " + ", ".join(missing) + f". Detected columns: {detected}")
    rows, errors = [], []
    for i, r in df.iterrows():
        try:
            ...
            qty = parse_num(r[qty_col]); ltp = parse_num(r[price_col])
            avg = parse_num(r[avg_col]) if avg_col else None
            pnl = parse_num(r[pnl_col]) if pnl_col else None
            if qty < 0 or ltp <= 0:
                errors.append(f"row {i+2}: invalid quantity/LTP for {etf}"); continue
    ...
    for r in rows:
        ...
        if key not in aggregated:
            aggregated[key] = dict(r); continue
        a = aggregated[key]
        old_qty = float(a["quantity"]); new_qty = float(r["quantity"]); total_qty = old_qty + new_qty
        a["quantity"] = total_qty; a["value"] = float(a["value"]) + float(r["value"])
        if total_qty > 0:
            a["average_price"] = (float(a["average_price"] or 0) * old_qty + float(r["average_price"] or 0) * new_qty) / total_qty
...
def build_reconciliation(snapshot, kite_rows, target_each):
    ...
    for etf in TOP_ETFS:
        r = by.get(etf, {"etf": etf, "quantity": 0.0, "average_price": None, "last_price": 0.0, "value": 0.0, "pnl": None, "isin": None})
        current = float(r["value"]); gap = float(target_each - current); price = float(r["last_price"] or 0.0)
...
# Latest automated GORS market refresh is the data-freshness truth.
...
with st.sidebar:
    ...
    if snapshot:
        ...
        st.number_input("💰 Kite Available Cash (₹)", min_value=0.0, value=0.0, step=100.0, key="sidebar_kite_cash", help="Set this before saving your first Kite snapshot.")
    ...
    if st.button("💾 Backup GORS DB", use_container_width=True):
        p = backup_database(); st.success(f"Backup created: {p}")
...
risk_state = decision["risk_state"]; TOP_ETFS = decision["top_etfs"]; target_exposure = decision["target_exposure"]; target_each = decision["target_each"]
risk_label = "RISK-ON" if risk_state == "RISK-ON" else "RISK-OFF 50%"
market_refresh_text = latest_market_refresh[0]["created_at"] if latest_market_refresh else "No automated refresh recorded"
...
c1.metric("Strategy", "FROZEN"); c2.metric("Risk State", risk_state); c3.metric("Target Exposure", money(target_exposure)); c4.metric("Target / ETF", money(target_each))
...
if uploaded is not None:
    ...
    try:
        kite_rows, raw_df = parse_kite_csv(raw); checksum = hashlib.sha256(raw).hexdigest(); portfolio_value = sum(float(r["value"]) for r in kite_rows)
        ...
        preview = pd.DataFrame(top3_rows)[["etf", "quantity", "last_price", "value"]].rename(columns={"etf":"ETF", "quantity":"Units", "last_price":"LTP", "value":"Value"})
        ...
        if latest_checksum and checksum == latest_checksum:
            ...
            if st.button("✅ Save New Verified Kite Snapshot", type="primary", use_container_width=True):
                sid = save_kite_snapshot(datetime.now().isoformat(timespec="seconds"), "Kite CSV", uploaded.name, cash, portfolio_value, len(raw_df), checksum, kite_rows, raw_csv=raw)
                ...
                backup = backup_database(); record_integrity("INFO", "db_backup", f"Recorded database persistence after Kite snapshot {sid}: {backup.name if backup else 'none'}")
                st.success(f"Snapshot #{sid} saved permanently with complete Kite holdings + original CSV. DB backup created."); st.rerun()
    except Exception as e:
        st.error(f"Kite CSV rejected: {e}")

if snapshot:
    st.markdown(f"<div class='kite-box'><b>🟢 Latest Kite snapshot automatically loaded from the persistent database</b><br>Snapshot #{snapshot['id']} • {snapshot['snapshot_time']} • {snapshot['file_name']} • checksum {str(snapshot['checksum'])[:12]}…<br><span style='font-size:.9rem'>This saved portfolio is used for Blocks B & C until you upload a newer Kite snapshot.</span></div>", unsafe_allow_html=True)
else:
...
if snapshot:
    recon = build_reconciliation(snapshot, kite_rows, target_each); current_total = float(recon["Current"].sum()); target_total = float(target_each * len(TOP_ETFS)); cash = float(snapshot["cash"])
    buy_value = float(recon.loc[recon["Action"] == "BUY", "Gap"].clip(lower=0).sum()); sell_value = float((-recon.loc[recon["Action"] == "SELL", "Gap"]).clip(lower=0).sum()); funding_gap = target_total - current_total - cash
    ...
    export_df = ticket_df[["ETF", "Action", "LTP", "Whole Units", "Estimated Order Value", "Target Gap", "Residual Gap"]].copy(); export_df.columns = ["ETF", "Action", "Kite LTP", "Whole Units", "Estimated Order Value", "Target Gap", "Residual Gap"]
    st.download_button("⬇️ Export Execution Ticket CSV", data=export_df.to_csv(index=False).encode("utf-8"), file_name=f"GORS_Execution_Ticket_{date.today().isoformat()}.csv", mime="text/csv")
