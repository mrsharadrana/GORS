...
def make_panel(days=260):
    ...
    return pd.DataFrame(
        {name: 100.0 + np.arange(days) * (0.20 + i * 0.03) for i, name in enumerate(gors_engine.TICKERS)},
        index=idx,
    )
...
def test_post_cutoff_data_cannot_change_historical_backtest():
    base = make_panel()
    cutoff = base.index[200]
    shocked = base.copy()
    shocked.loc[cutoff + pd.offsets.BDay(1):] *= 0.01

    a = safe.run(base, as_of=cutoff + pd.Timedelta(days=1))
    b = safe.run(shocked, as_of=cutoff + pd.Timedelta(days=1))

    assert a["cutoff"] == cutoff
    assert b["cutoff"] == cutoff
    pd.testing.assert_frame_equal(a["state"], b["state"])
...
def test_cutoff_uses_india_calendar_for_aware_timestamp():
    ...
    # 23:30 UTC is 05:00 IST on the following calendar date.
...
def test_data_before_cutoff_can_change_result():
    ...
    cutoff = base.index[200]
    changed = base.copy()
    changed.loc[cutoff, "NIFTY"] *= 0.80

    a = safe.run(base, as_of=cutoff + pd.Timedelta(days=1))
    b = safe.run(changed, as_of=cutoff + pd.Timedelta(days=1))
