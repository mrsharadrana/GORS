...
def sample_panel(days=240):
    ...
    return pd.DataFrame({name: 100 + np.arange(days) * (0.15 + i * 0.03) for i, name in enumerate(engine.TICKERS)}, index=idx)
...
def test_shared_decision_returns_exact_engine_top3(monkeypatch):
    panel = sample_panel(); monkeypatch.setattr(gors_decision, "load_market_data", lambda: panel)
    as_of = panel.index[-1] + pd.Timedelta(days=1)
    decision = gors_decision.get_current_gors_decision(as_of=as_of)
    cutoff = engine.latest_completed_common_date(panel, as_of=as_of); scoped = panel.loc[:cutoff]
...
def test_top3_validation_requires_exactly_three(monkeypatch):
    panel = sample_panel(); monkeypatch.setattr(gors_decision, "load_market_data", lambda: panel)
    monkeypatch.setattr(gors_decision, "calculate_gors_signal", lambda panel, as_of: {"top3": ["A", "B", "C", "D"], "signal_date": "2026-01-01", "ranking_date": "2026-01-01"})
    with pytest.raises(RuntimeError, match="expected exactly 3 Top-3 ETFs"): gors_decision.get_current_gors_decision(as_of=panel.index[-1])


def test_cutoff_changes_risk_state_instead_of_using_full_history():
    panel = risk_panel(); crash_date = panel.index[150]
    before = gors_decision.get_current_gors_decision(panel=panel, as_of=crash_date)
    after = gors_decision.get_current_gors_decision(panel=panel, as_of=crash_date + pd.Timedelta(days=1))
    assert before["signal_date"] == (crash_date - pd.offsets.BDay(1)).date().isoformat()
    assert before["risk_state"] == "RISK ON"
    assert after["signal_date"] == crash_date.date().isoformat()
    assert after["risk_state"] == "RISK OFF"
    assert after["target_exposure_pct"] == pytest.approx(engine.RISK_OFF_EXPOSURE)
